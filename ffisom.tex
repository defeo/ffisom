...
First, let us consider the case where $n$ is odd. Suppose $j\neq0, 1728$. Let 
...
\]
Similarly, we define $\widehat{\psi}_{m,a}, \psi_m$ the lift of $\psi_{m,a},
\psi_m$ respectively. Now let $L_m = L[X]/(\widehat{\psi}_{m,a}(X))$ be the 
extension of degree $(m²-1)/2$ and let $R_m = L_m[Y]/(Y^2 - X^3 + \widehat{A}X +
\widehat{B}$. We also write $\Theta = \bar{X}$ and $\Gamma = \bar{Y}$.

Now, let us define for $s\in(\Z/m\Z)^{\times}$ the action
\begin{equation}
\rho_s : \Theta \mapsto ([s]\widehat{P})_X
\end{equation}
which define an autormorphism of $L_m/L$. Let $G = \lbrace{\rho_s : 1 \leq s
\leq (m-1)/2}\rbrace$ subgroup of the Galois group $\text{Gal}(L_m/L)$. If we 
...
$\zeta_6$
    ...
    \FOR{$j_0\in\F_q$ \AND $j_0\neq0,1728$}
        ...
        \STATE $t_E \leftarrow \text{Trace}(E)$
        \IF{$t_E\in\mathcal{T}_m$}
            \RETURN $E$
        \ELSIF{$-t_E\in\mathcal{T}_m$}
