...
\item Case 1. We take $f$ of degree $m$ in $\F_q[x]$ and we define
  ...
  The same holds for $\tau_d$, with this time a cost in $O(T_Q\log(cd))$,
...
\item\label{frob2} Case 3. We take $f$ of degree $m$ in $\F_q[X]$, and
  ...
  in Case 2 above), in $O(\MM(mn)\log(q))$ operations. In order to compute
  ...
  We can then compute $\sigma_d,\alpha_d,\mu_d$ using respectively
  $O((T_{Q,n}+T_S)\log(d))$,
  $O((T_{Q,n}+T_S)\log(cd))$,
  and $O((T_{Q,n}+T_S)\log(d) + \MM(nm)\log(c)\log(d))$ operations,
...
\item Case 4. As a particular case of the previous item, suppose that
  ...
  $O(\MM(n)\log(q))$; in the last step, the cost $T_S$ of modular
  ...
  $S$ can be reduced to $O(n\MM(m) + m\MM(n))$ operations.  Up to these
...
\item Case 5. As another particular case of Case 3, suppose that $g$
  ...
  coefficients $d_j(x)=c_j(x)^{q^i}$ in $O(T_{Q,n})$ operations; we now have to
  ...
  this takes $T_{Q}+O(\MM(m)\log(r))$ operations; since the first stage of the
  algorithm took $O( T_{Q,n})$ operations, we can take $T_S=O(\MM(mr))$.  To
