...
\subparagraph{Case 1.}
  ...
  and $O(\MM(m) \log(q) + (T_Q + \MM(m)\log(c))\log(d))$.
...
\subparagraph{Case 2.}
  ...
  %Up to this modification,
  %the costs of computing $\sigma_d, \tau_d, \mu_d$ are the same as above. 
  We can therefore compute $\sigma_d, \tau_d, \mu_d$ using respectively
  $O(\MM(m) \log(q) + \MM(r) \log(d))$,
  $O(\MM(m) \log(q) + \MM(r) \log(d c))$,
  and $O(\MM(m) \log(q) + (\MM(r) + \MM(m)\log(c))\log(d))$.
...
\subparagraph{Case 4.}
  ...
  $O((\MM(m)+\MM(n))\log(q) + (T_{Q,n}+T_{Q',m})\log(cd))$,
...
\subparagraph{Case 5.}
  ...
  coefficients $d_j(x)=c_j(x)^{q^i}$ in $O(\MM(m)\log(q)+T_{Q,n})$ operations;
  we now have to compute $\sum_{j=0}^{n-1} d_j(x) \zeta_i^j$.
  ...
  algorithm took $O( T_{Q,n})$ operations, we can take $T_S=O(\MM(mr))$.
  
  To initiate the procedure, the algorithm also needs to compute
  $\alpha_1=\alpha^{\lfloor q/r\rfloor}$, using $O(\log(q))$
