...
\item Given $\delta\in\phi(k)$, compute $\phi^{-1}(\delta)$.
\end{itemize}
...
The two generic solutions we review for this problem are basic linear
algebra, and an algorithm based modular composition. %
While the former is very simple and effective, the latter has
considerably lower space and time asymptotic complexity. %
We also briefly review a solution, due to Kaltofen and
Shoup~\cite{kaltofen+shoup98}, that only applies when $\alpha$ is a
normal element; while this solution is not generic, some embedding
description algorithms (e.g., Rains' cyclotomic algorithm) naturally
produce normal generators, it is thus natural to include it in our
review.
...
operation can be sped up by pre-computing, e.g., an LU decomposition
for the matrix. %
...
However this cost can be counted as a pre-computation if we perform an
LU decomposition; in this case, the cost for evaluating $\phi$ or its
inverse drops down to $O(mn)$ operations. %
At any rate, the biggest drawback of this approach is the large memory
complexity: indeed storing the computed matrices requires $O(mn)$
elements of $\F_q$.
...
bilinear form by
\[\ang{\gamma,\delta}_k = \trace(\gamma\delta),\]
...
handling. %
In this case, $\phi$ does not preserve trace, indeed, if $n=[K:\F_q]$,
\begin{equation*}
...
\end{equation*}
We have thus shown that $\phi^t\circ H\circ\phi$ is the identity map.
...
\subsection{Algorithms specific to normal bases}
...
We apply $n-1$ times the $q$-th power to Eq.~\eqref{eq:eqnorm}, and apply $u$ to 
...
We can then rewrite Eq.~\eqref{eq:systemnormal} as
\begin{equation}
...
\section{More of Rains' algorithm}
...
%  LocalWords:  automorphism morphism
