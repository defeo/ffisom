...
An elliptic curve $E/L$ defined over a field $L$ is an equation of the
form
\begin{equation*}
...
$\eta_{\lambda,S}(P)=0$. To handle these corner cases, we give more
general definitions of elliptic periods in
Appendix~\ref{app:elliptic-curves}. In the rest of this section, we
focus on establishing when the elliptic period $\eta_{\lambda,S}(P)$
generates the same field extension as $x(P)$.

\begin{proposition}
  With the same notation as in Lemma~\ref{lemma:ellperiod}, for any
  point $P$ in the eigenspace of $\lambda$, the abscissa $x(P)$ is a
  normal element of $\F_{q^r}$ if and only if $\eta_{\lambda,S}(P)$ is.
\end{proposition}
\begin{proof}
  \todo{This is true, isn't it?}
\end{proof}
...
these implementations can be used to solve the conversion to a normal basis
coordinates problem. For a complete description of the factorization algorithm,
we redirect the reader to the section 3 of \cite{}. We focus on describing their 
implementation to help solve our problem.

The problem is as follow. Given an irreducible polynomial $f\in\F_q[x]$ of
degree $n$, a normal element $\eta$ and an element $\gamma\in\F_q[x]/f(x)$, we 
wish to compute $c_0,\dots,c_{n-1}\in\F_q$ such that :
\begin{equation}
\label{eq:eqnorm}
\gamma=c_0\eta+\dots+c_{n-1}\eta^{q^{n-1}}.
\end{equation}
They claim and prove the following result.
...
See~\cite[Theorem 6]{}
\end{proof}
Let $Q$ be the $n \times n$ matrix representing the $q$-th power map $\sigma$ on
$\F_q[x]/f(x)$, with respect to the natural power basis. Let $u$ be a projection
map from $\F_{q^n}$ to $\F_q$. We successively apply the $q$-th power $n-1$ 
times to (\ref{eq:eqnorm}), then we apply $u$ so we obtain the following 
\begin{equation}
u(\gamma^{q^j})=\sum_{i=0}^{n-1}{c_iu(\eta^{q^{i+j}})}\text{ for }0\leq j<n.
\end{equation}
To find the $c_i$'s, we will use the Algorithm AP from \cite{}. 

The inputs are an element $\gamma\in\F_q[X]/f(X)$, $u$ and $Q$ as previously 
described. It computes 
\begin{equation}
\label{eq;outputAP}
\vec{u}^{T}Q^i\vec{\gamma}
\end{equation}
for $0\leq i < k$ for $k = o(n)$, with $\vec{\gamma}$ the vector of the
coordinates of $\gamma$ and $\vec{u}$ the vector representing $u$ in the power
basis.

The algorithm is parameterized by a constant $\beta$, $0\leq\beta\leq1$. If we
set $t=\lceil{\gamma^\beta}\rceil$ and $m=\lceil{k/t}\rceil$, we can rewrite
(\ref{eq:outputAP}) as
\begin{equation}
\label{eq:outputAP2}
(\vec{u}^TQ^{ij})\cdot(Q^i\vec{\gamma})
\end{equation}
for $0\leq j<m)$ and $0\leq i<t$.  The steps are :
\begin{enumerate}
    \item Compute the vectors $Q^i\vec{\gamma}$ for $0\leq i<t$ by a squaring
algorithm $t-1$ times.

    \item Using \cite[Algorithm 5.2]{}, we compute $x^{q^t}\bmod f$.

    \item Compute the vectors $\vec{u}Q^{tj}$ for $0\leq j<m$ by computing
$m-1$ transposed modular compositions for the right multiplications by $Q^t$.

    \item Using the values computed in steps $1$ and $2$, we compute all the
values of(\ref{eq:outputAP2}) by multiplying $m\times n$ matrices by $n\times t$
matrices. 
\end{enumerate}
