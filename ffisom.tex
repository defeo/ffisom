...
Let $\F_q \subset k$ be a finite extension of degree $m$ where $k = \F_q[X] / (f)$ with $f$ monic 
irreducible of degree $m$. Given a divisor $s$ of $m$ we want to find an intermediate extension 
$\F_q \subset L \subset k$ of degree $s$ over $\F_q$. More precisely, we want to compute a monic 
irreducible polynomial $g \in \F_q[X]$ of degree $s$, and a polynomial $h \in k$ such that $x 
\mapsto h(x)$ is an embedding $L = \F_q[X] / (g) \hookrightarrow k$. We proceed as follows.

Let $a \in k$ be a non-constant random polynomial. Then $a$ has a minimal polynomial of degree $m$ 
...
\begin{equation}
	...
	\trace_{k/L}(a) = a + a^{q^s} + \cdots + a^{q^{(m/s - 1)s}}
\end{equation}
has a minimal polynomial of degree $s$ over $\F_q$ with high probability as well. This means we can 
compute the desired polynomials $h = \trace_{k/L}(a)$ and $g = \minpoly(h)$ after $O(1)$ random 
trials.
\begin{proposition}
	\label{prop:subfield}
	Let $\F_q \subset k$ be a finite extension of degree $m$, and let $s$ be a divisor of $m$. 
...
\begin{proof}
	...
	see \cite{shoup93}. It remains to compute the trace (\ref{equ:trace-simple}). This can be done 
	recursively as follows. Let $T_i(a) = a + a^{q^s} + \cdots + a^{q^{(i - 1)s}}$, and $\xi_i = 
	x^{q^{is}}$. Then
	\[
	\xi_j = 
	\begin{cases}
	...
	T_j(a) = 
	\begin{cases}
	T_{j / 2}(a) + T_{j / 2}(a)^{q^{js/2}} & j \text{ even} \\
	a + T_{j - 1}(a)^{q^s} & j \text{ odd}
	\end{cases}
	\]
	Raising a polynomial $b \in k$ to the power of $q^j$ is the same computing the composition 
	$b(\xi_j)$. This means that each step of the above recursion is done using $O(1)$ modular 
	composition in $k$. The depth of the recursion is $\log(i)$. Therefore, given the initial value 
	$\x_1$, $T_i(a)$ is computed using $O(\CC(m)\log(i))$ operations in $\F_q$. To compute the 
	initial value $\xi_1 = x^{q^s}$ we first compute $x^q$ using the usual binary-powering 
	algorithm at cost of $O(\MM(n)\log(q))$ operations in $\F_q$. Then the rest is done using the 
	same recursive technique as above at the cost of $O(\CC(m)\log(s))$ operations in $\F_q$. 
	Therefore, computing the trace $\trace_{k/L}(a) = T_{m/s}(a)$ takes $O(\CC(m)\log(m) + 
	\MM(m)\log(q))$ operations in $\F_q$.
\end{proof}
...
to the extension degree $n$, Kaltofen and Shoup~\cite{kaltofen+shoup97} 
give an efficient algorithm which is as follows.

\begin{algorithm}[Kaltofen-Shoup EDF for extension fields]
	\label{alg:ks}
	\begin{algorithmic}[1]
		\REQUIRE A polynomial $f$ of degree $m$ with irreducible factors of degree $d$ over $K$.
		\ENSURE An irreducible factor of $f$ over $K$.
		\STATE If $\deg f = d$ return $f$.
		\STATE Take a random polynomial $h\in K[X]$ of degree less than $\deg 
		f$,
		\STATE\label{alg:ks-pseudotrace} Compute $\displaystyle\dot{h} 
		\leftarrow \sum_{i=0}^{\deg 
			g-1} h^{q^i} \mod f$,
		\IF{$q$ is an even power $q=2^e$}
		\STATE\label{alg:ks:even} Compute $\displaystyle\ddot{h} \leftarrow 
		\sum_{i=0}^{e-1} \dot{h}^{2^i}\mod f$
		\ELSE
		\STATE\label{alg:ks:odd} Compute $\ddot{h} \leftarrow \dot{h}^{(q-1)/2}\mod f$
		\ENDIF
		\STATE\label{alg:ks:gcd} Compute $f_0\leftarrow\gcd(\ddot{h},f)$ and 
		$f_1\leftarrow\gcd(\ddot{h}-1,f)$ and $f_{-1}\leftarrow f/(f_0f_1)$,
...
\begin{proposition}
	...
	Kaltofen-Shoup algorithm finds a factor of $f$ in $K=\F_q[Y]/g(Y)$
...
As the first special case, we consider factoring cyclotomic polynomials. Let 
$\Phi_s(X)$ be the $s$-th cyclotomic polynomial, and let $n=\deg g$ and 
$d=\order_sq^n$.


\begin{proposition}
  \label{prop:cyclo}
  Given the cyclotomic polynomial $\Phi_s(X)$, and $g$ of degree and $n$,
  an irreducible factor of $\Phi_s$ of degree $d$ over $K=\F_q[Y]/g(Y)$ 
  can be computed using $O(sn(\log(d)+\log(n))\log(s) + \MM(sn)\log(sn) + 
  \MM(sn)\log(q))$ operations in $\F_q$.
\end{proposition}
\begin{proof}
  We use the Algorithm \ref{alg:ks} and the optimization techniques used in
  \cite{shoup94}. We start by analyzing the complexity of a single recursive 
  call, and we let $t=\deg f$.

  Since we have $X^s = 1$, the trace-like function in  
  Step~\ref{alg:ks-pseudotrace} can be computed modulu $X^s - 1$. As remarked 
  by Shoup~\cite{shoup94}, powerings of the form $x\mapsto x^{q^j} \mod X^s-1$ 
  boil down to simple permutations of the coefficients, which is done using 
  $O(sn)$ operations each. Thus, the total cost for  
  Step~\ref{alg:ks-pseudotrace} is $O(sn(\log(d)+\log(n)))$.

  After computing the trace modulu $X^s - 1$ we have to reduce it modulu $f$
  which is done by a division with remainder using $O(\MM(sn))$ operations.
  The exponentiations in Steps~\ref{alg:ks:even} and~\ref{alg:ks:odd} are done in
  $O(\MM(tn)\log(q))$ operations. Finally, the gcds in Step.~\ref{alg:ks:gcd} are computed in 
  $O(\MM(tn)\log(tn))$.
  ...
  complexity of Steps~\ref{alg:ks-pseudotrace} and reduction of the trace$\pmod f$ by
...
extensions. Let $r$ be a prime power and let $g$ be an irreducible factor of the $r$-th
cyclotomic polynomial $\Phi_r$ with $s = \deg(g)$. Denote $\F_q[Z] / g(Z)$ by $\F_q(\zeta)$ where 
$\zeta = z$ is the image of $Z$ in the quotient. Given an $r$-th power $a \in \F_q(\zeta)$ we want 
to compute an $r$-th root $a^{1 / r}$, or equivalently a linear factor of $X^r - a$ over 
$\F_q(\zeta)$.
\begin{proposition}
	\label{prop:root-fpz}
...
\begin{remark}
	...
	subsequent sections, at most quadratic in $r$. This also forces the term $r\CC(s)$ is 
	Proposition \ref{prop:root-fpz} to be subquadratic, hence keeping the complexity in both cases 
	at most quadratic. 
\end{remark}
\begin{proof}
Again we discuss Step \ref{alg:ks-pseudotrace} of Algorithm \ref{alg:ks} which is the dominant 
step. For clarity, we rewrite the recursion relations in \cite{kaltofen+shoup97}. Let
\[ \beta_i = h^q + h^{q^2} + \cdots + h^{q^i}, \quad \xi_i = x^{q^i}, \quad \zeta_i = z^{q^i}. \]
Then we have $\beta_1 = h^q$, $\xi_1 = x^q$, $\zeta_1 = z^q$, and
\[
\beta_j = 
\begin{cases}
...
\end{equation}
Now we analyze the running of computing $\dot{h} = h + \beta_{s - 1}$. First note that we always 
have $z^r = 1, x^r = a$. This makes it possible to keep the identities simple and do the reductions 
...
\begin{itemize}
	...
	\item If $s \in O(r^{(3 - \omega) / 2})$ we can first reduce $z^{q^j \bmod r}$ modulo $g(Z)$, 
	and then do $n$ modular compositions at a total cost of $O(\MM(r) + n\CC(s))$. Otherwise, we  
	reduce $c_l(z^{q^j \bmod r})$ modulo $g(Z)$ for all $0 \le l < n$ at a total cost of 
	$O(n\MM(r))$.
	\item $O(n\MM(s))$ for evaluating $\beta_j$ at $x^{q^j}$ using Horner's method.
	\item $O(\MM(rs))$ for reducing (\ref{equation:betaj}) modulo $f$.
\end{itemize}
The depth of the recursion is $\log(s)$, hence $\dot{h}$ can be computed in $O(\MM(s)\log(q) + 
