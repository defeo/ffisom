...
\end{equation}
Then we pick $\lambda\in(\Z/\ell\Z)^{\ast}$ such that $\order_\ell(\lambda)= $ 
and, since $\lambda\mu = q$, we check that $\order_\ell(q/\lambda)$ does not 
divide $r$. If these conditions are met, we write $t=\lambda+q/\lambda\bmod\ell$ 
and check if it is a good trace candidate using Hasse theorem. More precisely, 
we verify that $\widehat{t}$ the lift of $t$ is actually in the Hasse's bound
\begin{equation}
