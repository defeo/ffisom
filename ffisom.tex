...
Lemma~\ref{thm:gaussian}, that is that elliptic periods are normal generators
of $\F_q[x(P)]$.
...
and the discussion below together with experimental evidence suggest
...
elliptic periods should generate $\F_{q^r}$ in general,
at least when $q$ and $r$ are prime, so it can be adapted to
...
of $q$ and $r$, but we found none so far.
\todo{give actual numbers, don't remember where I've put them}

Another idea to produce a counterexample would be to reduce
an elliptic curve defined over $\Q$ with suitable torsion
and division polynomial.
\todo{add experiment data}
