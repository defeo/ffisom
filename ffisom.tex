...
\subsubsection{Computing $v$-th roots in $F_1[\zeta]$}
...
\subsubsection{Computing the isomorphism}
...
\end{equation}
operations on $\F_q$, using the algorithm in \cite{doliskanischost2011}, which is
...
\]
Since $r|m$, the terms in $r$ are dominated by those in $m$. Finally, $\CC(m)$ 
