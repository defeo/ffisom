...
order to cover a wider range of parameter ranges.
And indeed Pinch~\cite{Pinch} showed how to use torsion points of elliptic
curves in place roots of unity.
Rains also considered this possiblity, but did not investigated it thoroughly
...
is very costly whereas the elliptic variant often leverages the need for
an extension and is therefore very competitive.
...
Throughout this section, $\ell$ will be a prime number different from $p$.
If $E$ is an elliptic curve defined over $\F_q$ this implies that the geometric
$\ell$-torsion $E[\ell]$ of $E$ is of rank $2$.
...
is an Elkies prime for $E$ if the charateristic polynomial of the Frobenius
endomorphism $\pi_q$ splits in $\Z/\ell\Z$:
\begin{equation}
...
then $E[\ell]$ splits into two eigenspaces for $\pi_q$
which are defined on extensions of $\F_q$ of degrees
$\order_l(\lambda)$ and $\order_l(\mu)$.
In the following we suppose that $r = \order_l(\lambda)$
and $\order_l(\mu) \not| r$
...
elliptic curves always come with non trivial automorphisms:
for $p > 3$, $\# \Aut(E)$ is $2$ for $j(E) \neq 0, 1728$,
$4$ for $j(E) = 1728$ and $6$ for $j(E) = 0$.

\begin{definition}
\label{definition:ellperiod}
Let $E/\F_q$ be an elliptic curve,
$\ell > 3$ an Elkies prime for $E$,
$\lambda$ an eigenvalue of $\pi_q$,
$r = \order_\ell(\lambda)$.,
and $P_\lambda \in E(\F_{q^r})$ a point in the eigenspace corresponding
to $\lambda$.

Suppose furthermore that $\# \Aut_{\F_{q^r}}(E)$ divides $\ell - 1$
and the existence of a subgroup $S$ of $(\Z/\ell\Z)^{\ast}$ such that
\begin{equation}
...
\begin{equation}
\eta(P_\lambda) = \sum_{\sigma\in S} {x \left([\sigma] P_\lambda \right)}
\end{equation}
\end{definition}
...
\begin{enumerate}
    ...
    \item $(\Z/\ell\Z)^{\ast}/\Aut(E) = \langle{\lambda}\rangle \times S$
for $S$ a subgroup of $(\Z/\ell\Z)^{\ast}$,
    \item $\order_\ell(\lambda) = r$ and $\order_\ell(\mu)\not|r$.
\end{enumerate}

Let $E/\F_q$ be an elliptic curve of trace $t$
and $P_\lambda \in E(\F_{q^r})$ in the eigenspace
corresponding to $\lambda$.

Then the elliptic period $\eta(P_\lambda)$ generates $\F_{q^r}$ over $\F_q$.
\end{theorem}

\begin{proof}
We are not sure this is true!
For sure, $\eta(P_\lambda)$ is not a normal element (in general).
