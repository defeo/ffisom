...
\begin{definition}
  ...
  Let $E/\F_q$ be an elliptic curve. Let $\ell>3$ be an Elkies prime
  for $E$, let $\lambda$ be an eigenvalue of $\pi$, and let
  $\langle P\rangle$ be the corresponding eigenspace.
...
\begin{proof}
  ...
  automorphism $\nu$ of $A_\lambda$. %
  Multiplication by $c$ is defined by rational fractions, hence its
  action on $\langle P\rangle/\{\pm 1\}$ is given by a polynomial $C$
  modulo $f_\lambda$. This proves the statement.
\end{proof}

We are now ready to state our lemma on elliptic periods.
...
and lift up to $A_\lambda$ implying the same linear relation for
$\mathrm{Tr}(x)$ and its Galois conjugates.
Therefore, if $x$ is normal in $A_\lambda$,
then $\eta_{\lambda,S}(P)$ is normal in $\F_q(x(P))$.
\end{proof}

We note that the converse is easily shown not to be true by experimentation.
There is also no relationship between the normality of $f_\lambda$
and that of $x(P)$ as an element of $\F_{q^r}$,
or between the normality of $x(P)$ and that of $\eta_{\lambda,S}(P)$.

Nonetheless, the lemma shows that heuristically
$\eta_{\lambda,S}(P)$ has a high probability of being normal and therefore
of generating $\F_q(x(P))$.
We can even quantify this probability using the following lemma.
\begin{lemma}
\label{lemma:euleralgebra}
Let $A_\lambda$ be a polynomially cyclic algebra of degree $dr$
over the finite field $\F_q$.
The number of normal elements in $A_\lambda$ is the same as the
number of normal elements in $\F_{q^{dr}}$, i.e.\
$\Phi_q(x^{dr}-1)$ where $\Phi_q$ is the Euler function
for polynomials.
\end{lemma}
\begin{proof}
There is at least one normal element in $A_\lambda$~\cite[Theorem 4]{Mihailescu2010825}
...
\item as the Galois group of $A_\lambda$ is cyclic, the decomposition
...
of $q$ and $r$, but as stated in the next subsection
...
Galois conjugates of $X \in A_\lambda$ such as the
norm\footnote{Note that this fails trivially for cyclotomic Gaussian periods}.
However for most elementary symmetric functions, except for the trace,
examples where this definition yields a non-generating element.
