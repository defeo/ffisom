...
a variant which is similar to the present one, in the specific case 2 with $s=1$ 
we describe below.
...
We are left with the problem of finding $\theta_1$, that is, an instance of Hilbert's theorem 90.
...
need $O(1)$ trials to find $\theta_1$.
...
computing an embedding $k \hookrightarrow K$ using Allombert's algorithm.
\begin{proposition}
	\label{proposition:XiDelta-updated}
...
cyclotomic field extensions $\F_q(\zeta), k(\zeta)$ of degree $s$ over $\F_q, k$ respectively, by 
obtaining an irreducible factor of the $v$th cyclotomic polynomial over $\F_q$. Then we obtain a  
non-$v$-adic residue $\eta \in F(\zeta)$. 
...
F(\alpha)$. The main result of this section is given in Theorem~\ref{theorem:isom-root} below.
By applying it to both $k$ and $K$, we immediately obtain an embedding description algorithm.

\begin{theorem}
	\label{theorem:isom-root}
	Let $r = v^d$ where $v \ne 2, p$ is a prime and $d$ is a positive integer. Also let $s$ be the 
	order of $q$ in $\Z / v\Z$. Given an extension $k/\F_q$ of degree $r$, a generator 
	$\alpha$ of $k$, such that its orbit under $\gal(k/\F_q)$ is uniquely defined,
        can be constructed at the cost of 
...
Raising $a$ to the power of $q^{is}$ gives
\[a^{q^{is}} = \sum_{j = 0}^{s - 1} a_j(x^{q^{is}})z^j = \sum_{j = 0}^{s - 1} a_j(\xi_i)z^j = 
a(\xi_i).\]
This can be done in $O(s\CC(r))$ operations in $\F_q$. Also, to compute $x^{q^s}$ we first compute 
$x^q$ using $O(\MM(r)\log(q))$ operations in $\F_q$, and then do $\log(s)$ modular compositions in 
$k$. Then $\lambda^{q^s}$ can be computed using $O(s\CC(r))$ operations in $\F_q$. Therefore, the 
...
Now, we can construct $\alpha$ in Theorem~\ref{theorem:isom-root} as follows. We first build the 
extension $k(\zeta) / \F_q(\zeta)$. Let $\eta$ be a non-$v$-adic residue in $\F_q(\zeta)$. 
Then $\eta$ is an $r$-power in $k(\zeta)$. To obtain an $r$-th root $\theta$ of $\eta$ we take $d$ 
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		\REQUIRE An extension $k / \F_q$ of degree prime-power $r = v^d$, with $v\ne2,p$.
		\ENSURE An element $\alpha \in k$, with uniquely defined Galois orbit, such that $k = \F_q(\alpha)$.
		\STATE\label{step:factor-cyclo} Factor the $v$th cyclotomic polynomial $\F_q$ to 
		build the extension $k(\zeta) / \F_q(\zeta)$;
		\STATE\label{step:non-v-res} Find a non-$v$-adic residue $\eta \in \F_q(\zeta)$;
