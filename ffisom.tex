...
to ensure that $\ell$ is an Elkies prime modulo $p$,
...
For $r=3$, $d = 2$ and $\ell = 2 d r + 1 = 13$, there exists a suitable
infinite family of elliptic curve which is quite easy to construct.
...
The kernel polynomials of the other elliptic curves might be of degree $6$,
but they can still be used, just as in the case of the cyclotomic Rains
algorithm with an auxiliary extension.
...
A few composite values of $\ell \geq 13$ for which rational elliptic curves
with a rational $\ell$-isogeny can also be considered.
For $\ell = 14, 15, 16, 18, 27$, the factorization of $\phi(\ell)/2$
gives suitable coprime values for $d$ and $r$, leaving only a couple of values.
