...
\begin{definition}
  ...
  $1728$. %
  \footnote{The definition is easily extended to deal with these two cases:
  one must quotient $S$ by $\Aut(E)$ or part of it depending on wheter
  it is (partially) contained in $\langle{\lambda}\rangle$ or not.}
...
Heuristically, it suggests that the
...
\begin{remark}
Using a generalized definiton of elliptic periods, it is possible to include for elliptic
curves with $j=0,1728$,
but it is straightforward to generalize this definition to correctly
