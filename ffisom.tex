...
\begin{abstract}
  ...
  $m = \deg f$ dividing $n = \deg g$, to compute an explicit description of a
  field embedding of $\F_q[X]/f$ into $\F_q[X]/g$. When $m = \deg n$,
...
in~$\MC(m) + O(m n \log q)$ operations
...
roughly be $\tildO(r^2)$ just like the other variants, the difference will be 
...
We define the division polynomials as follow. We have $\psi_\ell\in\Z[x,y,A,B]$ and 
\begin{align*}
\psi_1 &=1,\\
\psi_2 &= 2y,\\
...
\begin{align*}
\psi_{2\ell+1}&=\psi_{\ell+2}\psi_{\ell}^{3}-\psi_{\ell-1}\psi^{3}_{\ell+1}
&&\text{ for }\ell\geq2,\\
\psi_{2\ell}&=\left(\frac{\psi_{\ell}}{2y}\right)\cdot(
\psi_{\ell+2}\psi^{2}_{\ell-1}-\psi_{\ell-2}\psi^{2}_{\ell+1})&&\text{ for }\ell\geq
3.
\end{align*}
More precisely, the roots of $\psi_\ell$ are exactly the abscissas of the
points in $E[\ell]$. Since $E[\ell]$ is either isomorphic to $\Z/\ell\Z\times\Z/\ell\Z$,
$\Z/\ell\Z$ or $0$, we have the following
\begin{equation}
...
the degree to be equal to $(\ell^2 - 1)/2$ and for that we have to introduce the
\emph{Elkies prime}.
...
\begin{remark}
If $\ell$ is an Elkies prime for $E$, then there exists a basis such that the 
Frobenius acts as a diagonal matrix on $E[\ell]$, on which the restriction of
...
\end{pmatrix}.
\end{equation}
For all integer $r$, the action Frobenius of $E/\F_{q^r}$ will be the $r$th
power of that matrix. Additionnaly, if we denote by $k =
\textup{min}(\order_\ell(\lambda),\order_\ell(\mu))$ then the extension $\F_{q^k}$ is
the smallest extension on which there are $\ell$-torsion points. Indeed, let $k = 
\order_\ell(\lambda)$ and $P$ be in the eigenspace of $\lambda$, then we have the following
\begin{equation}
...
Let $E/\F_q$ be an elliptic curve and let $\ell>2$ be an Elkies prime for $E$.

Let $j\neq0, 1728$. Let $\lambda$ be an eigenvalue of $\pi_q \bmod \ell$. Suppose there 
exists a subgroup $S$ of $(\Z/\ell\Z)^{\ast}$ such that 
\begin{equation}
(\Z/\ell\Z)^{\ast}/\lbrace{\pm1}\rbrace=\langle{\lambda}\rangle\times S
\end{equation}
Then for every $P\in E[\ell]$ in the eigenspace generated by $\lambda$, we define
the elliptic periods as follow
\begin{equation}
\eta_{\lambda}(P) = \sum_{\sigma\in S}{\left([\sigma]P\right)_X}
\end{equation}
where $P_X$ denotes the abscissa of the point $P$.

For the cases $j = 0$ or $1728$, we have to take in account the size
of $\Aut(E)$. Namely, we shall assume that its order divides $\varphi(\ell)$
then we identify it to a subgroup of $(\Z/\ell\Z)^{\ast}$. Assume there exists a
...
Let $\ell$ be a prime different from $p$, $r$ an odd integer and $t$ an integer 
such that 
...
\begin{equation}
\mathcal{O}_L/\mathfrak{p} \simeq \F_q.
\end{equation}
We denotes by $\widehat{E}$ the lift of $E$ in $L$,
\[
\widehat{E} : Y^2 = X^3 + \widehat{A}X + \widehat{B}
\]
Similarly, we define $\widehat{\psi}_{\ell,\lambda}, \widehat{\psi_\ell}$ the lifts of 
$\psi_{\ell,\lambda}, \psi_\ell$ respectively. Now let $L_\ell := 
L[X]/(\widehat{\psi}_\ell(X))$ be the extension of degree $(\ell^2-1)/2$ and let 
$R_\ell := L_\ell[Y]/(Y^2 - X^3 + \widehat{A}X + \widehat{B})$. We also write $\Theta
:=\bar{X}$ and $\Gamma:=\bar{Y}$, $P:=(\Theta, \Gamma)$ will denote the generic 
point of $E[\ell]$.

For $o\in(\Z/\ell\Z)^{\ast}$ we write
\begin{equation}
\rho_o : \Theta \mapsto ([o]\widehat{P})_X
\end{equation}
which define an autormorphism of $L_\ell/L$. Let $G := \lbrace{\rho_o : 1 \leq o
\leq (\ell-1)/2}\rbrace$, subgroup of the Galois group $\text{Gal}(L_\ell/L)$; we also 
write $L_0 := L_\ell^G$. With that definition we can write 
\begin{equation}
\widehat{\psi}_{\ell,\lambda}=\prod_{o=1}^{\tfrac{\ell-1}{2}}{(X-\rho_o(\Theta))}
\end{equation}
with coefficient in $L_0$, consequently $L_\ell = L_0[T]/(\widehat{\psi}_{\ell,\lambda}(T))$.
\emph{(Moreover, since $G$ is cyclic, for each $o\in G$ there exists a polynomial
$\widehat{g}_o$ of degree $\text{deg}(\widehat{g}_o) < (\ell-1)/2$ such that
...
\]
actually forms a normal basis of $L_\ell/L_0$.
Therefore, for any intermediate extension $L'/L_0$ of $L_\ell/L_0$, the trace
$\text{Tr}_{L_\ell/L'}(\Theta)$ along with its conjugates also form a normal basis
...
\begin{tikzpicture}
    ...
    \node[above= of Lpr] (Lm) {$L_\ell = L(E[\ell]) = L_0[X]/(\widehat{\psi}_{\ell,\lambda}(X))$};
    \node[above= of Rpr] (Rm) {$R_\ell$};
    \node[above= of Fqa] (A0) {$\mathcal{A}_0 = \F_q(E[\ell])$};
...
\end{tikzpicture}
\end{center}
where $\Phi_\ell(X,j(\widehat{E}))$ is an equation of the modular curve $X_0(\ell)$.

We recall that, since $\ell$ is an Elkies prime for $E$ by definition, we can 
find a generator $c\in(\Z/\ell\Z)^{\ast}$ such that $\lambda = c^{r'}$ for 
$r'=(\ell-1)/(2n)$, and we have $S=\langle{s}\rangle$ with $s=c^r$. 
For $0\leq i<q$, we define 
\begin{equation}
\eta_i = \sum_{\sigma\in S}{g_{\sigma}(g_{\lambda^i}(\theta))}
\end{equation}
reduction modulo $\mathfrak{p}$ of $\widehat{\eta}_i$. For $\ell\geq3$ odd, the
discriminant of $\psi_\ell$ satisfies the following relation
\begin{equation}
\text{Disc}(\psi_\ell) = (-1)^{(\ell-1)/2}\ell^{(\ell^2 - 3)/2}(-\Delta)^{(\ell^2 - 1)(\ell^2 -
3)/24}
\end{equation}
...
there would exists linear relation between the roots of $\psi_{\ell,\lambda}$.
...
characteristic $0$, the action of $\langle{\lambda}\rangle$ on the $\eta_i$ is cyclic,
as a direct consequence the $\eta_i$ are conjugates and $M(T)$ the reduction of
$\widehat{M}(T)$ is the minimal polynomial of $\eta_0 = \eta_\lambda(P)$ and is of
degree $r$. This concludes the proof of the theorem.
\end{proof}
...
We need to find an integer $\ell\neq p$ such that there exist an element of order $r$
modulo $\ell$. The first condition will be that $r|\varphi(\ell)$. We also need 
that the cofactor $\varphi(\ell)/r$ is prime with $r$, so it ensures that 
$(\Z/\ell\Z)^{\ast}$ splits as we want, namely 
\begin{equation}
(\Z/\ell\Z)^{\ast}/\Aut(E)=\langle{\lambda}\rangle\times S \text{ or } 
(\Z/\ell\Z)^{\ast}=\langle{\lambda}\rangle\times S
\end{equation}
for $S$ a subgroup of $(\Z/\ell\Z)^{\ast}$ and $\lambda$ the eigenvalue of $\pi_q$ of
order $r$ modulo $\ell$. We also wish for $\mu$, the other eigenvalue of $\pi_q$ to
be of order not dividing $r$, so that there is not subfields of $\F_{q^r}$
containing $\ell$-torsion points and, consequently, those in $\F_{q^r}$ are in the 
eigenspace of $\lambda$.

To pick a good elliptic curve, we shall first pick some trace candidate modulo 
$\ell$. Since we need $\ell$ to be an Elkies prime for the future elliptic curve, the
polynomial in $(\Z/\ell\Z)$ shall be of the form
\begin{equation}
(X - \lambda)(X - \mu) = X^2 - (\lambda + \mu)X + \lambda\mu \bmod \ell.
\end{equation}
Then we pick $\lambda\in(\Z/\ell\Z)^{\ast}$ such that $\order_\ell(\lambda) = r$ and,
since $\lambda\mu = q$, we check that $\order_\ell(q/\lambda)\not| r$. If these conditions are
met, we write $t = \lambda + q/\lambda \bmod \ell$ and check if it is a good trace candidate
...
In summary, we need to find an integer $\ell$ such that :
\begin{itemize}
    \item $\ell$ is prime,
...
\end{equation}
the good trace candidates for a prime $\ell$, then the routine to find a good 
