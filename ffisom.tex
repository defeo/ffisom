...
to date being in~\cite{LeGall14}.
...
Brent--Kung algorithm~\cite{brent+kung}, which takes $O(s^{1/2}\MM(sm)
+ s^{(\omega+1)/2}\MM(m))$ operations in $\F_q$.  In the binary RAM
...
algorithm. Hence, we will restrict to an algebraic complexity model,
denote by $\CC(s,m)$ the complexity of modular composition in $k[Z]$,
and use $O(s^{1/2}\MM(sm) + s^{(\omega+1)/2}\MM(m))$ as an upper
bound.  We will use $\CC(s)$ as a shorthand notation for $\CC(s,1)$.

\paragraph{Evaluating Frobenius and trace-like maps}

We will make a heavy use of a technique originating
in~\cite{von1992computing}. Given $\alpha\in k$, for an extension
$\F_q\subset k$ of degree $m$, we are interested in computing the
values $\alpha^{q^d}$ and $\sum_{i=0}^d \alpha^{q^i}$ for some integer
$d$. A simple binary powering approach would yield a complexity of
$O(d\MM(m)\log(q))$ operations over $\F_q$; however, using fast
modular composition, it is possible to obtain a better complexity
bound in some cases.
...
\begin{proof}
  ...
  \begin{equation*}
    q^i = A_it + B_i, \quad 0\le B_i < t,
  \end{equation*}
  together with $\mu_i=\alpha^{A_i}$. We have $(q^d-1)/t = \lfloor q^d
  / t \rfloor$, hence our goal is to compute $\mu_d$.

  We proceed as in Proposition~\ref{prop:trace-like}, maintaining
  simultaneously the quantity $\xi_i=x^{q^i}$.  We start by computing
  $\mu_1=\alpha^{A_1} = \alpha^{\lfloor q / t\rfloor}$ and $\xi_1 =
  x^q$ by a simple binary powering, at a cost of $O(\MM(m)\log(q)))$
  operations.
  Assuming that $\mu_i, \mu_j$ and $\xi_i,\xi_j$ are known, a direct
  calculation shows that
  \[ \mu_{i+j} = 
    \mu_j^{q^i} \mu_i^{B_j}\alpha^{\lfloor B_iB_j / t \rfloor},\quad
    \xi_{i+j} = \xi_j^{q^i}.
  \]
  We compute $\mu_j^{q^i}$ and $\xi_j^{q^i}$ by means of modular
  compositions with $\xi_i$, as in Proposition~\ref{prop:trace-like},
  for a cost $O(\CC(m))$. The exponents $B_j$ and $\lfloor B_iB_j / t
  \rfloor$ are less than $t$, so computing the corresponding powers
  naively costs $O(\MM(m)\log(t))$ operations.

  We can compute $\mu_d$ by a binary scheme using $O(\log(d))$ such
  steps, so the overall cost is $O(\MM(m)\log(t)\log(d) +
  \CC(m)\log(d) + \MM(m)\log(q))$.
\end{proof}
...
\begin{proposition}
	...
	Let $\F_q \subset k$ be a finite extension of degree $m$, and
        ...
        its image in $k$ using $\CC(m)$ operations.
\end{proposition}
\begin{proof}
...
We refer the reader to the original paper~\cite{kaltofen+shoup97}
for the correctness of the Kaltofen--Shoup algorithm. We redo the
complexity analysis below, to adapt to our framework.

\todo{We should say somewhere that (some) runtimes are Las Vegas}

\todo{$\log n$ or $\log(n)$? I'd vote for the second one}
\begin{proposition}
...
\end{proposition}
\begin{proof}
	The dominating step  is the computation of the trace-like function at
	Step~\ref{alg:ks-pseudotrace}. By using a divide-and-conquer
	strategy, Kaltofen and Shoup show in~\cite{kaltofen+shoup97} how to compute it in
	$\log (dm)$ steps, using at each step
	\begin{itemize}
		\item $O(s)$ modular compositions in degree $m$ in $\F_q[X]$,
		\item $O(1)$ modular compositions in degree $s$ in $k[Z]$,
	...
	a half, thus the overall complexity is dominated by that of the first call.
\end{proof}
...
\end{proposition}
\begin{proof}
  We use Algorithm~\ref{alg:ks} with $h=\Phi_s$, and the optimization
  techniques given in~\cite{shoup94}. We start by analyzing the
  complexity of a single recursive call, using the same notation as in
...
As the next special case of Algorithm \ref{alg:ks}, we consider taking
roots in cyclotomic extensions. Let $r$ be a prime power and let $f$
be an irreducible factor of the $r$-th cyclotomic polynomial $\Phi_r$,
with $s = \deg h$. Denote $\F_q[Z] / f(Z)$ by $\F_q(\zeta)$, where
$\zeta$ is the image of $Z$ in the quotient. Given an $r$-th power
$\alpha \in \F_q(\zeta)$ we want to compute an $r$-th root $\alpha^{1
  / r}$, or equivalently a linear factor of $X^r - \alpha$ over
$\F_q(\zeta)$.
\begin{proposition}
  \label{prop:root-fpz}
  ...
  $$O(\MM(s)\log(q)\log(r) + r\CC(s)\log(s) + \MM(rs)\log(s)\log(r))$$
  or
...
%% \end{note}
\begin{proof}
We adapt Algorithm~\ref{alg:ks} to get a linear factor of the
polynomial $X^r-\alpha$.  Again, we discuss
Step~\ref{alg:ks-pseudotrace}, which is the dominant step. Let $f \in
\F_q[Z]$ be the defining polynomial of $\F_q(\zeta)$, let $h$ be a
factor of $X^r-\alpha$ of degree $n$, and let $x$ be the image of $X$
modulo $h$. For clarity, we rewrite the recursion relations in
\cite{kaltofen+shoup97}. Let
\[ 
\beta_i = a_0^q + a_0^{q^2} + \cdots + a_0^{q^i};%% , \quad \xi_i = x^{q^i}, \quad \zeta_i = \zeta^{q^i}
\]
Then we have $\beta_1 = a_0^q$ and
\[
\beta_i = 
\begin{cases}
\beta_{i / 2} + \beta_{i / 2}^{q^{i / 2}} & i \text{ even} \\
\beta_1 + \beta_{i - 1}^q & i \text{ odd};
\end{cases}
\]
We also define $\alpha_i=a^{\lfloor q^i / r\rfloor} \in \F_q(\zeta)$,
so that $\alpha_1 = a^{\lfloor q / r\rfloor}$ and, as in Proposition~\ref{prop:large-power},
\[
\alpha_i=
\begin{cases}
\alpha_{i / 2}^{q^{i/2}} \alpha_{i/2}^{B_{i/2}} a^{\lfloor B_{i/2}^2/r \rfloor} & i \text{ even} \\[1mm]
\alpha_{i-1}^q \alpha_1^{B_{i-1}} a^{\lfloor B_1 B_{i-1}/r \rfloor} & i \text{ odd,}
\end{cases} \quad
\text{with } B_i=q^i \bmod r \text{~for all $i$.}
\]
Computing $\alpha_1$ takes $O(\MM(s)\log(q))$ operations in $\F_q$.
In the first case above, assuming $\alpha_{i/2}$ is known, we can
deduce $\alpha_i$ for the cost of one exponentiation with power
$q^{i/2}$, and $2$ exponentiations with powers less than $r$. The
former is done as in Proposition~\ref{prop:cyclo}, in time
$O(\MM(r))$; the latter by a binary method, in time
$O(\MM(s)\log(r))$, for a total of $O(\MM(r)+\MM(s)\log(r))$. The cost
for the second case is similar.

For a positive integer $i$ and $\beta$ in $\F_q(\zeta)[x]$, writing
$\beta = \sum_{i=0}^{n-1} c_\ell(\zeta) x^\ell$, we have
\begin{equation}
\label{equation:betaj}
\beta^{q^i} = \left( \sum_{\ell = 0}^{n - 1}c_\ell(\zeta)x^\ell \right)^{q^i} = \sum_{\ell = 0}^{n - 
	1}c_\ell(\zeta^{q^i})(x^{q^i})^\ell.
\end{equation}
Assuming that $\alpha_i$ is known, we analyze the cost of this computation.
The bulk of the calculation is done modulo $X^r-a$.
\begin{itemize}
\item We first compute $\xi_i=X^{q^i} \bmod X^r-a$ as $\alpha_i X^{q \bmod r}$
  (this is free since $\alpha_i$ is known).
\item To compute $c_0(\zeta_i),\dots,c_{n-1}(\zeta_i)$, we can first
  compute $Z^{q^i \bmod r}$ modulo $f(Z)$, and then do $n$ modular
  compositions at a total cost of $O(\MM(r) + n\CC(s))$, or compute
  $c_\ell(Z^{q^i \bmod r}) \bmod (Z^r-1)$
  and reduce it
 modulo $f(Z)$, for all $0 \le \ell < n$, at
  a total cost of $O(n\MM(r))$.
\item $O(n\MM(s))$ for evaluating $\sum_{\ell = 0}^{n -
  1}c_\ell(\zeta^{q^i}) X^\ell$ at $\xi_i$ using Horner's method, with
  all calculations done modulo $X^r-a$.
\item $O(\MM(rs))$ for reducing the result modulo $h(X)$; this is the
  cost of a Euclidean division in degree $r$ in $\F_q(\zeta)[X]$.
\end{itemize}
With $i=1$, this allows us to compute $\beta_1$; for higher values
of $i$, we can compute $\beta_i$ (together with all
required $\alpha_i$) in $O(\log(s))$ such steps. 
As a result, $a_1$ can be computed in either
$$O(\MM(s)\log(q) + n\CC(s)\log(s) + \MM(rs)\log(s))$$
or
$$O(\MM(s)\log(q) + n\MM(r)\log(s) + \MM(rs)\log(s))$$ operations in
$\F_q$. Since the depth of the recursion in Algorithm~\ref{alg:ks} is
$\log(r)$, and the degree $n$ is halved each time, we deduce the
desired result.
\end{proof}
...
We now focus on the complexity analysis
of the computation of such a solution $\theta$
to Hilbert's theorem 90.

\subsubsection{Algorithms relying on linear algebra}
\label{sec:algor-rely-line}
Before presenting quasi-quadratic algorithms based on evaluation formulas,
we briefly describe Allombert's original algorithm~\cite{Allombert02}
and its revision~\cite{Allombert02-rev}, % and another variation,
both of them relying on linear algebra.%
\footnote{
Note that all these variants can be directly be applied to any extension
degree $r$ as long as $p \nmid r$
and do not require $r$ to be a prime power.
Nevertheless, in practice, it is more efficient to perform computations
...
and no precise complexity analysis was performed,
we figured the details from the PARI/GP source code
and perform the complexity analysis here.
We also propose another variant relying on linear algebra.

\paragraph{Allombert's original algorithm}
The original version of Allombert's algorithm~\cite{Allombert02}
computes the matrix of the Frobenius automorphism $\sigma$ of $k/\F_q$
using $O(\MM(r) \log(q) + r \MM(r))$ operations in  $\F_q$
and then an eigenvalue of $\sigma$ for $\zeta$ over $\F_q(\zeta)$
using linear algebra at a cost of $O(\MMM(rs))$ operations in $\F_q$
yielding the following complexity.

\begin{proposition}
Using Allombert's original algorithm,
a solution $\theta$ to Hilbert's theorem 90 can be computed in
$O(s \MM(r) \log(q) + r \MM(r) + \MMM(rs))$ operations in $\F_q$.
\end{proposition}
...
on an element in the kernel of $\hat{h}$ over $k$, with
linear algebra now taking place over $\F_q$ rather than $\F_q(\zeta)$.
The details on the computation of $\hat{h}$ were extracted
from PARI/GP source code and yield the following complexity.

\begin{proposition}
Using Allombert's revised algorithm,
a solution $\theta$ to Hilbert's theorem 90 can be computed in
$O(\MM(r) \log(q) + s r \MM(r) + \MMM(r))$ operations in $\F_q$.
\end{proposition}
...
To get the matrix of $\hat{h}$ over $k$,
one first computes the powers $x^{q^i}$ for $0 \leq i \leq s$
...
From them and the matrix of $\sigma$, on can iteratively compute
the powers $x^{j q^i}$ for $2 \leq j \leq r$
for a total cost of $O(s r \MM(r))$ operations in $\F_q$,
and iteratively compute the matrix of $\hat{h}$
for an additional total cost of $O(s r^2)$ operations in $\F_q$,
accounting for the scalar multiplications by the coefficients of $h$.
The total cost is therefore dominated by $O(s r \MM(r))$ operations in $\F_q$.

Given the matrix of $\hat{h}$ over $k$,
computing an element in its kernel
costs $O(\MMM(r))$ operations in $\F_q$.

The final evaluation of $\hat{h'}$ is done using Eq.~\eqref{eq:hprime}
and the matrix of $\sigma$ of for Frobenius computation
for a cost of $O(s r^2)$ operations in $\F_q$.
%(or $O(s \MM(r) \log(q))$ operations in $\F_q$ using modular exponentiation).
\end{proof}


\paragraph{Using Paterson--Stockmeyer algorithm}
An obvious improvement to the evaluation of $\hat{h}$, though not implemented in PARI/GP,
can be achieved with the Paterson--Stockmeyer algorithm~\cite{paterson_stockmeyer}
which relies on matrix multiplication.
%(or even Horner's rule).
Evaluations of $\sigma$ can also be done directly using modular exponentiations
as for the initial evaluation of $x^q$.
This approach yields the following complexity.
...
\subsubsection{Algorithms relying on evaluation formulas}
\label{sec:algor-rely-eval}
...
\begin{proof}
  ...
  We first compute $\hat{g}(a)$ using the
...
\begin{proposition}
  ...
  %Algorithm~\ref{alog:allombert} is correct.
  Given $k,K$ of degree $r$
...
\begin{proof}
  ...
  given by Proposition~\ref{prop:root-fpz} and can not be neglected.
  ...
  \item If $s\in O(\todo{})$, we use the algorithm described in
    ...
    $O(\CC(r)r^{(3-\omega)\lvert\beta-1/2\rvert+1-\beta}+(s+r^\beta)\MM(r)\log(q)+r\CC(s)\log(s)+\MM(rs)\log(s)\log(r))$
    operations.
  \item Otherwise, we use the algorithm described in Case~3. Combining
...
efficiently $v$-th roots in $k(\zeta)$ and is addressed below.
...
First note that $v$ divides $q^{rs}-1$ where $rs$ is the degree of the extension
$k(\zeta)$ over $\F_q$.
Following~\cite[Algorithm~3]{doliskanischost2011},
one reduces the root extraction in $k(\zeta)$ to a root extraction
in a smaller extension of $\F_q$.%
...
The minimal polynomial, root extraction in the subfield
and embedding computation corresponding to Steps 9, 10 and 11
of~\cite[Algorithm~3]{doliskanischost2011}, have a negligible cost.
Therefore, the complexity of the present variation
of~\cite[Algorithm~3]{doliskanischost2011} will be
...
\begin{lemma}
  ...
  \end{equation*}
  Then, 
  for any point $P$ in the eigenspace of $\lambda$, the period
  $\eta_{\lambda,S}(P)$ is in $\F_{q^r}$, and its minimal polynomial
  does not depend on the choice of $P$.%
  \footnote{
  With our construction, $r$ divides $(\ell-1)/2$,
  so that its $2$-adic valuation is at most one less than the $2$-adic
  valuation of $(\ell - 1)$.
  It is still possible to generate an extension with degree having the same
  $2$-adic valuation as $(\ell-1)$ if $-1 \in \langle\lambda\rangle$.
  To do so, Definition~\ref{definition:ellperiod} be modified
  to use ordinates rather than abscissas.
  }
\end{lemma}
\begin{proof}
...
for $r$ odd.%
\footnote{
If $r$ is even, % and divides $(\ell-1)/2$,
it might be needed to go a quadratic extension of $\F_{q^r}$
or use a quadratic twist for torsion points to be defined.
%If $r$ has the same $2$-adic valuation as $(\ell-1)$, abscissas
%should be replaced by ordinates when computing elliptic periods.
}
Given $k$, $K$ and $r$,
\begin{enumerate}
...
labeled \emph{``Original''} in the graphs. %
...
almost perfectly with our \emph{``Case 1''} curve. %
...
Section~\ref{sec:algor-rely-eval}, one can't fail to notice that the one
