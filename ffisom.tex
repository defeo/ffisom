...
It should still be checked that modulo $p$ the kernel polynomial
does not split into more factor as over the rational
and the corresponding counterexample explicitly computed.
...
Nevertheless, they can still be used, in the same way as the cyclotomic
Rains algorithm with an auxiliary extension degree $s = 2$.
We now treat the other prime values for $\ell \geq 13$.

For $\ell = 17$, there are two rational elliptic curves
...
This implies that $2$ would divide both $(\ell-1)/(2 r s)$ and $r s$.
...
For $\ell = 19$, there are two rational elliptic curves
with a rational $19$-isogeny:
curves~\texttt{361A1} and \texttt{361A2},
and both their kernel polynomials are irreducible.
Therefore, they both can be used with $r s = (\ell-1)/2 = 2\cdot3$.

For $\ell = 37$, there are two rational elliptic curves
with a rational $37$-isogeny.
The first one is curve~\texttt{1225H1}
% in Cremona's tables~\cite{cremona_tables}
...
The only possibility for $r$ is therefore $r = 2$.
The second curve is curve~\texttt{1225H2}
%in Cremona's tables~\cite{cremona_tables}
%with $j$-invariant $-7\cdot137^3\cdot2083^3$
and its kernel polynomial is irreducible.
Therefore, it can be used with $r s = (\ell-1)/2 = 2\cdot3^2$.

For each $\ell = 43, 67, 163$, there are two rational elliptic curves
with a rational $\ell$-isogeny 
and they all have irreducible kernel polynomials.
Therefore, they can be used with $r s = (\ell-1)/2 = 3\cdot7, 3\cdot11, 3^4$.
...
there are eight rational elliptic curves with a rational $15$-isogeny.
For curves~\texttt{50a3}, \texttt{50a4}, \texttt{50b1} and \texttt{50b2},
the part of the kernel polynomial corresponding to primitive $15$-torsion
splits into two quadratic factors, whereas,
for curves~\texttt{50a1}, \texttt{50a2}, \texttt{50b3} and \texttt{50b4},
this part is a quartic irreducible.
Therefore, the former four curves can not be used because $2$ would divide
both $r = 2$ and $\phi(25)/(2 r) = 2$,
whereas the latter four ones can be used with $r s = 2^2$.

%For $\ell = 16$, with $\phi(\ell)/2 = 2^2$,
%the multiplicative group is not cyclic.

For $\ell = 21$, with $\phi(21)/2 = 2\cdot3$,
there are eight rational elliptic curves with a rational $21$-isogeny
All of them have a rational point of $3$-torsion,
and curve~\texttt{162b1} is the unique rational elliptic curve
with $21$-torsion over a cubic extension of $\Q$~\cite{najman_cubic}.
For curves~\texttt{162b1} and ~\texttt{162c2},
the part of the kernel polynomial corresponding to primitive $21$-torsion
splits into two cubic factors, whereas,
for the six other curves, this part is a sextic irreducible.
Therefore, the two former curves, can only be used with $r = 3$ (and $d = 2$),
whereas the six latter ones can be used with $r s = 2\cdot3$.
...
and compute periods for $r s = 10$.

For $\ell = 27$, with $\phi(27)/2 = 3^2$,
there are two rational elliptic curves with a rational $27$-isogeny.
For both curves, the part of the kernel polynomial corresponding to
primitive $27$-torsion is irreducible of degree $9$.
Therefore, the both can be used with $r s = 3^2$.
