...
similar to the Gaussian periods form a normal basis of $\F_q^r$, then we 
...
taken into account in the definition of the elliptic periods. We shall focus
...
such that $(m/r,r) = 1$, $E/\F_q$ an elliptic curve defined over $\F_q$, $\ell$
the Elkies prime for $E/\F_q$ satisfying the conditions of theorem
...
defined Galois orbit.
    ...
    \UNTIL{$P\neq\mathcal{O}$}
    \STATE $\eta\leftarrow\sum_{\sigma\in S}{\left([\sigma]P\right)_X^{\#\Aut(E)/2}}$
    \RETURN $\eta$
    \end{algorithmic}
\end{algorithm}
