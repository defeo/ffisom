...
Step~\ref{step:factor-cyclo} is done using by Algorithm~\ref{alg:ks} which, by Prop.~\ref{prop:cyclo}, takes $O(\MM(v)\log(vq))$ operations in $\F_q$. We do Step~\ref{step:non-v-res} by taking random elements in $\F_q(\zeta)$ until a non-$v$-adic residue is found; using Shoup's variation~\cite[Lemma~15]{shoup94} on the proof of Prop.~\ref{prop:large-power}, we can do this using $O(\MM(v)\log(s) + \MM(s)\log(v)\log(s) + \MM(s)\log(q))$ operations in 
