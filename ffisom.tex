...
The few following candidates for $r$ and $\ell$ can not be used.
...
and for $r = 7$ and $\ell = 29$ there is no infinite family of curves
...
Indeed, rational elliptic  curves with rational $\ell$-isogenies exist only for
a finite set of values of $\ell$~\cite{Mazur1974}
corresponding to non-cuspidal rational points on $X_0(N)$.
Moreover, among the suitable prime values for $\ell \geq 11$,
only $\ell = 13$ provides an infinite family~\cite{Lozano-Robledo2013}.
For $\ell = 11, 17, 19, 43, 67, 163$, there are only finitely
many rational elliptic curves equipped with a rational $\ell$-isogeny;
moreover, except for the $j$-invariant $j = -17\cdot373^3/2^{17}$
associated with $\ell = 17$, all kernel polynomials $f_\lambda$
are irreducible of degree $(\ell-1)/2$.
For $\ell = 13$, as aforementioned, there is an infinite family of rational
...
is defined over an extension of $\Q$ of degree $(\ell-1)/3 = 12$
