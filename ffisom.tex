...
\sqrt{q}$, then the complexity is $\O(m^2(\log{q} + \log{m}))$.
\end{proposition}
\begin{proof}
...
\[
O(\MM(m)(m\log{m}+r\log{q}-\log{\ell}+
(\ell\log{\ell})/r)+\CC(m)\log{m}).
\]
Since in practice an additionnal requirement for $\ell$ is $\ell\in 
O(\sqrt{q})$, see section \ref{sec:selection}, the terms in $\log{\ell}$ 
are absorbed by those in $\log{m}$. Morever, since $r|m$, the terms in $r$ are 
dominated by those in $m$. Finally, $\CC(m)$ is subquadratic in $m$, it is 
dominated by the rest of the formula. Therefore, we end up with a complexity of 
$O(m\MM(m)(\log{q} + \log{m}))$.
\end{proof}
