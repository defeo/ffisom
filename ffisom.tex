...
These algorithms, based on principle radically different from
...
to our knowledge, this is the fist detailed and complete complexity
...
\subsection{Embedding description}
...
we give a variant of these algorithms with a significantly lower complexity.
...
Allombert's algorithms.

\paragraph{Case 0: Allombert's algorithms}
Both Allombert's algorithm rely on linear algebra.

The first version of the algorithm simply computes an eigenvalue of
$\sigma$ for $\zeta$ over $\F_q(\zeta)$ at a cost of
$O(\MM(r) \log q + r \MM(r) + \MMM(rs))$.

In his revised algorithm, Allombert remarks that
from an element in the kernel of $\hat{h}$ over $k$,
a solution to Hilbert's theorem 90 can be deduced
by evaluating $h'(S)=h(S)/(S-\zeta)$ over $k[\zeta]$.
...
\begin{proposition}
Using Allombert's algorithm, a solution $\theta$ to
Hilbert's theorem 90 can be computed in
$O(s \MM(r) \log(q) + r \MM(r) + \sqrt{s} \MMM(r))$ operations.
\end{proposition}

\begin{proof}
After computing the matrix of $\sigma$ over $k$ at a cost of
$O(\MM(r) \log(q) + r \MM(r))$ operations, one can compute
the matrix of $\hat{h}$ in $O(\sqrt{s} \MMM(r))$ operations
...
at a cost of $O(s r^2)$ using the matrix of $\sigma$
(or $O(s \MM(r) \log q)$ using modular exponentiation),
and deduce the powers $x^{j q^i}$ for $2 \leq j \leq r$
at a cost $O(s r \MM(r))$,
in order to iteratively compute the matrix of $\hat{h}$
for an additional cost of $O(s r^2)$ for scalar
multiplications by the coefficients of $h$.
The total cost being $O(s r \MM(r))$.
}

Computing an element in the kernel of $\hat{h}$
over $k$ costs $O(\MMM(r))$ operations.

The final evaluation of $\hat{h'}$ can be done
naively in $O(s \MM(r) \log(q))$ using modular exponentiation
(or $O(s r^2)$ using the matrix of $\sigma$).
\end{proof}
...
First note that if $\vu$ is coprime with $q-1$ (that is $t$ defined above is $0$)  then a modular exponentiation by $\vu^{-1}$ in $k(\zeta)$ trivially yields a solution.

Let's now suppose that $\vu$ divides $q-1$.
Then one reduces the root extraction in $k(\zeta)$ to a root extraction in a smaller extension of $\F_p$ using~\cite[Algorithm~3]{doliskanischost2011}.
...
computes $\lambda = a^{p^s' - 1}/\vu}$ where $s'$ is the order of $p$
modulo $\nu$ (note that $s'$ divides $s$)
...
$1 + \lambda + \lambda^{1+p^{s'}} + \cdots + \lambda^{1+p^{s'}+\cdots+p^{s-2s'}}$
...
\todo{oops, $s \neq s'$ so $\zeta^{p^s'} \neq \zeta$}
\[ \lambda^{p^{i s'}} = \sum_{j = 0}^{s - 1} \lambda_j(x^{p^{i s'}}) \zeta^j = \sum_{j = 0}^{s - 1} \lambda_j(\xi_i) \zeta^j = 
\lambda(\xi_i)\]
where $\xi_i = x^{p^{is'}}$
using $O(s\CC(r))$ operations in $\F_q$.
Also, to compute $x^{q^s}$ we first compute 
$x^q$ using $O(\MM(r)\log(q))$ operations in $\F_q$, and then do $\log(s)$ modular compositions in  $k$.
Then for any $\lambda \in k(\zeta)$, $\lambda^{q^s}$ can be computed using $O(s\CC(r))$ operations in $\F_q$. Therefore, the 
...
$\Phi_q(x^{dr}-1) = O(q^{dr})$ where $\Phi_q$ is the Euler function
