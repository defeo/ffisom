...
\end{equation}
Set $h'=h/(S-\zeta)$, then the morphism $\hat\Theta$ can be evaluated
as $\hat{h'}\circ\hat{g}$, the advantage being that $g$ has
coefficients in $\F_q$, rather than in $\F_q(\zeta)$. %
Furthermore, if we set $h(S)=\zeta^s+\sum_{i=0}^{s-1}h_iS^i$, we can
...
\begin{equation}
  ...
  \end{aligned}\right.
\end{equation}
Indeed, Horner's rule shows that $h_{-1}'=h$, and by direct
calculation we find that $(S-\zeta)\cdot h' = h'_{-1}$.
...
\begin{proposition}
  ...
  \begin{itemize}
  \item $O(s\CC(r)\log(r)+\M(r)\log(q)+\MM(sr)\log(s)\log(r))$
