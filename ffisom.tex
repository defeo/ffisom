...
the slightly stronger one, that $\MM(mn)$ is $O(m^{1+\varepsilon}
...
be done in time $O(t s^{\omega-1})$, by computing $1,g,\dots,g^{s-1}$
...
\mu_d=\alpha^{\lfloor q^d/c\rfloor}.$$ A direct binary powering approach
...
$x$ is the image of $X$ modulo $f$. In the following items, we discuss
...
\begin{itemize}
\item Case 1. We take $f$ of degree $m$ in $\F_q[x]$ and we define
  $Q=\F_q[X]/f(X)$. We let $x$ be the image of $X$ in $Q$, and we
  start by computing $x^q$, using $O(\MM(m)\log(q))$ operations in
  $\F_q$.

  For $i \ge 0$, given $\xi_i = x^{q^i}$ and $\beta$ in $Q$, we can
  compute $\beta^{q^i}$ as $\beta^{q^i}=\beta(\xi_i)$, in time
  $T_Q=O(m^{(\omega+1)/2})$; in particular, this allows us to
  ...
  The same holds for $\tau_d$, with this time a cost $O(T_Q\log(cd))$,
  since we have to compute $\xi_c$ first. For $\mu_d$, the formula for
  $\mu_{i+j}$ shows that we can obtain it by means of a modular
  composition (to compute $\mu_j^{q^i}=\mu_j(\xi_i)$), together with
  two exponentiations of indices less than $c$, which are done using
  $O(\log(c))$ multiplications. The cost is thus $O((T_Q +
  \MM(m)\log(c))\log(d))$.

\item Case 2. As a particular case of the above, $f$ is a factor of
  degree $m$ of the $r$th cyclotomic polynomial, for some prime $r$.

  An alternative solution to obtain $\beta^{q^i}$, for some $\beta \in
  Q$, computes $\beta(X^{q^i \bmod r}) \bmod (X^r-1)$ and reduces it
  modulo $f(X)$; thus, we can take $T_Q=O(\MM(r))$. Up to this
  modification, the costs of computing $\sigma_d,\alpha_d,\mu_d$ are
  the same as above. 

\item\label{frob2} Case 3. We take $f$ of degree $m$ in $\F_q[X]$, and
  $g$ monic of degree $n$ in $Q[Z]$, where we write $Q=\F_q[X]/f(X)$;
  we now define $S=Q[Z]/g(Z)$.

  We let $x$ and $z$ be the respective images of $X$ and $Z$ in $Q$,
  and as a first step, we compute $z^q$ (and $x^q$, unless $f$ is as
  in Case 2 above), in time $O(\MM(mn)\log(q))$. In order to compute
  the quantities $\sigma_d,\tau_d,\mu_d$, we apply the same strategy
  as above; the key factor for complexity is thus the cost of
  computing $\beta^{q^i}$, for $\beta$ in $Q$, given $\zeta_i =
  ...
  We use an algorithm by Kaltofen and Shoup~\cite{kaltofen+shoup97},
  ...
  We can then compute $\sigma_d,\alpha_d,\mu_d$ in respective times 
  $O((T_{Q,n}+T_S)\log(d))$,
  $O((T_{Q,n}+T_S)\log(cd))$,
  and $O((T_{Q,n}+T_S)\log(d) + \MM(nm)\log(c)\log(d)),$
  where $O(\MM(mn))$ is the cost of one multiplication in $S$.

\item Case 4. As a particular case of the previous item, suppose that
  $g$ is in $\F_q[Z]$. Then, the cost for computing $z^q$ is
  $O(\MM(n)\log(q))$; the last step, the cost $T_S$ of modular
  composition in $S$ is now that of $m$ modular compositions in degree
  $n$ (with the same argument), as detailed in
  Note~\ref{note:multimc}. Similarly, the cost of multiplication in
  $S$ can be reduced to $O(n\MM(m) + m\MM(n))$.  Up to these
  modifications, the runtimes for computing $\sigma_d,\alpha_d,\mu_d$
  remain the same.

\item Case 5. As another particular case of Case 3, suppose that $g$
  is a monic factor of degree $n$ of a polynomial of the form $Z^r-a$,
  for some $a$ in $Q$.  

  For $\beta$ as above, suppose that we have already computed all
  coefficients $d_j(x)=c_j(x)^{q^i}$ in time $O(T_{Q,n})$; we now have to
  compute $\sum_{j=0}^{n-1} d_j(x) \zeta_i^j$.

  We first do the calculation modulo $Z^r-a$ rather than modulo $g$;
  that is, we compute $\sum_{j=0}^{n-1} d_j(x) \gamma_i^j$ where
  $\gamma_i=\gamma^{q^i}$ and $\gamma^r = a$. Because $\gamma^r=a$, we
  have $\gamma_i = \alpha_i \gamma^{q^i \bmod r}$, with $\alpha_i =
  \alpha^{\lfloor q^i/r\rfloor}$. If we assume that $\alpha_i$ is
  known, we can compute $\sum_{j=0}^{n-1} d_j(x) \gamma_i^j$ using
  ...
  chain, we must thus compute the corresponding $\alpha_i$'s as well;
  this takes time $T_{Q}+O(\MM(m)\log(r))$; since the first stage of the
  algorithm took time $O( T_{Q,n})$, we can take $T_S=O(\MM(mr))$.  To
  initiate the procedure, the algorithm also needs to compute
  $\alpha_1=q^{\lfloor q/r\rfloor}$, using $O(\log(q))$
  multiplications in $Q$.
\end{itemize}
...
integer and let $s$ be the order of $q$ in $\Z / v\Z$.  We actually
assume that $d \ge 2$, since this will be the case in all cases where
...
\le \sqrt{r}$. Therefore, the complexity of the present variation
of~\cite[Algorithm~3]{doliskanischost2011} will be
$O(s^{(\omega-1)/2}r^{(\omega+1)/2}\log(r) + s\MM(r)\log(q))$ operations in $\F_q$.
\todo{make this a proposition}
...
r^{(\omega+1)/2 + (1-\beta)(\omega-1)/2}+r^\beta\MM(r)\log(q))$.
