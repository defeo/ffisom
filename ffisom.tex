...
By applying the subfield technique, introduced in Section \ref{sec:introduction}, we assume the 
following reduced version of the embedding problem in the entire section: given extensions $k, K$  
of the same degree $r$ where $r$ is a prime power, construct an embedding $k \hookrightarrow K$.
...
an estimate of $O(r^3)$ operations in $\F_q$. The idea of the algorithm is as follows. First, if 
...
cyclotomic polynomial. Also let $\Delta$ denote the group of automorphisms 
$\Aut(\F_q[\zeta]/\F_q)$. Then we have $\F_q[\zeta]^\Delta = \F_q$. Let $\F_q[\zeta][c^{1/r}]$ 
denote the quotient $\F_q[\zeta][Y]/(Y^r - c)$ such that $c^{1/r}$ is the residue class of $Y$. For 
a given element $c \in T_{\F_q}$, where $T_{\F_q}$ is the Teichm\"{u}ller subgroup \footnote{See 
the reference for details.} of $\F_q[\zeta]$, the action of $\Delta$ can be extended over the ring 
$\F_q[\zeta][c^{1/r}]$ by sending $c^{1/r}$ to powers $(c^{1/r})^\ell$ for certain integers $\ell$. 
If $c_1, c_2 \in T_{\F_q}$ have the same order then we can find an isomorphism 
$\F_q[\zeta][c_1^{1/r}] 
\cong \F_q[\zeta][c_2^{1/r}]$ that is identity on $\F_q[\zeta]$ and preserves the action of 
$\Delta$. In fact since $c_1$ and $c_2$ both generate the same subgroups, there is an integer $j > 
0$ such that $c_1 = c_2^j$, and the isomorphism is given by
\[
\begin{array}{lrll}
...
\end{array}
\]
If $c$ is a generator of $T_{\F_q}$ then we can show that $\F_q[\zeta][c^{1/r}]^\Delta$ is a field 
extension of $\F_q$ of degree $r$. Also, we can find an element $\gamma \in \F_q[\zeta]$ such that 
$c = \gamma^r$ belongs to $\F_q[\zeta]^*$ and generates $T_{\F_q}$. This gives an isomorphism 
$\F_q[\zeta] \cong \F_q[\zeta][c^{1/r}]$ that is identity on $\F_q[\zeta]$, sends $\gamma$ to 
$c^{1/r}$, and preserve the action of $\Delta$. From this isomorphism we get the induced 
isomorphism $\F_q \cong \F_q[\zeta][c^{1/r}]^\Delta$. Therefore, given extensions $k, K$ of $\F_q$ 
of degree $r$, we have built a chain
\begin{equation*}
	\begin{tikzpicture}
		\node (Fc1) {$\F_q[\zeta][c_1^{1/r}]^\Delta$};
		\node[right = of Fc1] (Fc2) {$\F_q[\zeta][c_2^{1/r}]^\Delta$};
		\node[left = of Fc1] (F1) {$k$};
		\node[right = of Fc2] (F2) {$K$};
...
\begin{enumerate}
	...
	\item find a generator $c_1, c_2$ of $T_k, T_K$
	\item find isomorphisms $\varphi_i: k \xrightarrow{\sim} \F_q[\zeta][c_1^{1/r}]^\Delta$ and 
	$\varphi_i: K \xrightarrow{\sim} \F_q[\zeta][c_2^{1/r}]^\Delta$
	\item compute the integer $j$ such that $c_1 = c_2^j$
	\item build the isomorphism $\psi: \F_q[\zeta][c_1^{1/r}]^\Delta \xrightarrow{\sim} 
	\F_q[\zeta][c_2^{1/r}]^\Delta$
...
simplicity/practicality. For example, one can replace the ring extension $\F_q[\zeta]$ with the 
field extension $\F_q[\zeta] = \F_q[X] / g(X)$ where $g$ is an irreducible factor of $\Phi_r$. This 
makes things simpler and more practical at the cost of factoring $\Phi_r$ which of course 
introduces indeterminism if one choses to use efficient factoring algorithms.
...
$\alpha_1 \in k[\zeta]$, and $\alpha_2 \in K[\zeta]$. Then $b = \alpha_1^r / \alpha_2^r$ is an 
$r$th power in $\F_q[\zeta]$ from which $b^{1/r}$ can be computed using a root extraction 
algorithm. The isomorphism $k[\zeta] \xrightarrow{\sim} K[\zeta]$ is then given by $\alpha_1 
...
linear algebra. Therefore, the algorithm performs $O(n^{\omega + 1})$ many operations in $F$.


\subsection{Fast variant}

In this section, we present a fast variant of Allombert's algorithm using an explicit solution to 
the Hilbert 90 Theorem. Let $[k: \F_q] = r$ where $r$ is a prime power. Let $h(Z)$ be an 
...
\end{equation}
The idea of building such tower using the Artin-Schreier polynomials can also be found in 
