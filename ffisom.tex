...
taken in account in the definition of the elliptic periods. In particular, the cases 
$j = 0, 1728$, which have automorphisms group of cardinality greater than $2$, 
shall be dealt more specifically. We will now define the \emph{elliptic
periods} and detail some of their properties in a theorem below. We shall focus
...
\cite{MiMoScho} and fall into the case $j\neq0,1728$ and we shall first focus only
this case.
...
of degree $(\ell^2 - 1)/2$. It also divides $psi_\ell$. Now let 
$L_\ell:=L[X]/(\widehat{\psi}_\ell(X))$ be the extension of degree 
$(\ell^2-1)/2$ and let $R_\ell:=L_\ell[Y]/(Y^2-X^3+\widehat{A}X+\widehat{B})$. 
