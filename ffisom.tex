...
\todo{equivalence with same minimal polynomial}
...
root finding for the embedding description step
...
This can be done deterministically in $O()$
and probabistically in $O()$.
...
as $\beta(\gamma) \pmod{g}$ theoretically in $O()$
using Kedlaya-Umans, and practically in $O()$
using Brent-Kung.

The second question can be answered by performing a modular
exponentiation to check that $\gamma^{q^{\deg f}} = 1$ in $O()$.

The second and third questions can be answered using
dual bases and power projection/transposed modular composition
to express $\delta \in K$ as a polynomial in $\beta$ in $O()$.
...
\begin{algorithm}
    ...
    \REQUIRE $m$ an integer as above, $q$ a prime power, $n$ the extension
            degree, $\mathcal{T}_m$, $q$ a generator of $\F_{q^n}^{\times}$
