...
\begin{algorithm}[Rains' cyclotomic algorithm variant]
  ...
  \begin{algorithmic}[1]
    ...
    \STATE Compute a factor $h$ of the $\ell$-th cyclotomic polynomial $\Phi_\ell$ over $\F_{q^r}\subset k$ using Algorithm~\ref{alg:cyclo}; 
    \STATE Compute $\bar{h} = \rev{h'}/\rev{h} \mod z^\ell$ in $k[[z]]$;
    \RETURN\label{algorithm:rains-cyclo:period} $\eta = \sum_{\sigma\in S}[\bar{h}]_\sigma$.
  \end{algorithmic}
\end{algorithm}
...
\end{proposition}
\begin{proof}
  \todo{Explain how you factor over a subfield $\F_{q^{rs}}$.}
