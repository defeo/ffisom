...
The few following candidates for $r$ and $\ell$ when $d = 2$ can not be used.
Indeed, for $r = 5$ and $\ell = 21$,% (which is not prime)
there exist four rational elliptic curves with a rational $21$-isogeny,
but none of them has $21$-torsion defined over a quintic extension of
$\Q$~\cite{gonzalez-jimenez_complete_2016} or one of degree $10$
(indeed the kernel polynomials have no factor of degree $5$ or $10$),
and for $r = 7$ and $\ell = 29$ there is no rational elliptic curve
with rational $29$-torsion~\cite{Mazur1974} (or elliptic curves defined
...
to non-cuspidal rational points on $X_0(\ell)$.
...
Among this family are the aforementioned curves
with $13$-torsion defined over a cubic extension.%
\footnote{The larger family of rational elliptic curves
with a rational $13$-isogeny is parametrized by
$j(t) = \frac{\left(t^2 + 5t + 13\right)\left(t^4 + 7t^3 + 20t^2 + 19t + 1\right)^3}{t}$~\cite{najman_cubic}.}
For $\ell = 17, 19$, the factorization of $(\ell-1)/2$ does not allow
coprime values for $d$ and $r$.
For $\ell = 37$, there is a couple of rational elliptic curves with a rational
$37$-isogeny.
The first one has $j$-invariant $-7\cdot11^3$
...
leaving no possibility for coprime $d$ and $r$.
The other one has $j$-invariant $-7\cdot137^3\cdot2083^3$
and its kernel polynomial $f_\lambda$ is irreducible.
For each $\ell = 43, 67, 163$, there is exactly one rational elliptic curve
with a rational $\ell$-isogeny
and the corresponding kernel polynomial $f_\lambda$ are all irreducible.
For these four last curves, the factorization of $(\ell-1)/2$ leaves
the possibility to define coprime values for $d$ and $r$.
One could still proceed as above and use such values hoping that
$f_\lambda$ would split as needed over $\F_p$.
The only non-prime value of interest apart from $21$ mentioned above
would be $\ell = 25 = 2^3\cdot3+1$ which could be suitable for $d=4$ and $r=3$.
Unfortunately, among the infinite family of rational elliptic curve with
a rational $25$-isogeny, none of them has $25$-torsion defined over a cubic
extension~\cite{najman_cubic}, and only a finite number might have it defined
over a sextic extension~\cite{Derickx201452}.
