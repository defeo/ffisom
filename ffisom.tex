...
that the cofactor $\varphi(m)/(2n)$, or $\varphi(m)/n$ if $n$ is even, is prime
with $n$, so it ensures that $(\Z/m\Z)^{\times}$ splits as we want, namely 
\begin{equation}
...
\end{equation}
still depending on the parity of $n$.
