...
construct an element $\theta_1 \in k[\zeta]$ such that $\theta_1$ and $\tau_1 = \theta_1^r$ are 
...
\[ k[\zeta] \xrightarrow{\sim} \F_q[\zeta][Y]/(Y^r - \tau_1) = \F_q[\zeta][\theta_1] \]
sending $\theta_1$ to $Y$. Here, $\F_q[\zeta][\theta_1]$ is represented by the quotient 
$\F_q[\zeta][Y]/(Y^r - \tau_1)$ in which $\theta_1$ denotes the image of $Y$. Doing the same for 
$K[\zeta]$, elements $\theta_2 \in K[\zeta]$ and $\tau_2 \in \F_q[\zeta]$ and an 
$\F_q[\zeta]$-isomorphism
\[ \F_q[\zeta][\theta_2] = \F_q[\zeta][Y]/(Y^r - \tau_2) \xrightarrow{\sim} K[\zeta], \]
that sends $Y$ to $\theta_2$, is obtained. The algorithm constructs $\theta_1, \theta_2$ in such a 
way that an integer $j > 0$ can be found such that
\[
\begin{array}{lrll}
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    \REQUIRE Prime powers $q=p^d$ and $r$, a bound $\bar{u}\in o(\sqrt[4]{q}/r)$;
