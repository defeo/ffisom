...
$v$-th root extractions in $k(\zeta)$. Then $\theta_1$ is a non-$v$-th power in $k(\zeta)^*$. Since 
$\eta$ is a non-$v$-th power, the polynomial $Y^r - \eta$ is irreducible over $\F_q(\zeta)$ and 
there is an $\F_q(\zeta)$-isomorphism 
...
until a non $v$-th power is found. Testing $v$-th power for
