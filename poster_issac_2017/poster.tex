\documentclass[20pt,a0paper,blockverticalspace=10mm]{tikzposter}
...
\definetitlestyle{WLogos}{
   ...
   (\titleposright,0.75\titlepostop+0.25*\titleposbottom)
   node[anchor=east]{\includegraphics[width=18em]{uwaterloo}}
...
\begin{columns}
  ...
  \block{Allombert's algorithm}{
    ...
    \begin{wrapfigure}[8]{r}{0.35\colwidth}
      \begin{tikzpicture}[node distance=5em,on grid,black!70]
        ...
        \node(Kz)[above=of K]{$K\otimes\F_q(\zeta)$};
        \draw[font=\small,auto]
        (Fq) edge node{$\sigma$} (Fqz)
        (Fq) edge (k)
        (Fqz) edge (kz)
        (k) edge node{$\sigma$} (kz)
        (Fq) edge (K)
        (Fqz) edge (Kz)
        (K) edge node{$\sigma$} (Kz);
      \end{tikzpicture}
  ...
  \block[bodyinnersep=0]{Experimental results}{
    ...
    \newlength{\mywidth}
    \setlength{\mywidth}{0.6\colwidth}
