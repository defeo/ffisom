...
void check(int opt){
  ...
  for (i = 1; i*(i+1) < 500; i+=1){
    ...
    mp_ptr Gptr = _nmod_vec_init(degP*degQ);
    nmod_vec_rand_dense(Gptr, state, degP*degQ, FP->P->mod);

    if (opt == 1){
      embeddings_tisomorphism(Fptr, Gptr, FP, FQ, FR);
      ...
      sage_output_assign_vec(Gptr, degP*degQ, "G");
      ...
      printf("S = quo(X)\n");
      printf("T = quo(Y)\n");
      printf("F == [add([ U(((S^j*T^i).lift()(0,0,x))) [k]*G[k] for k in range(R.degree())]) for j in range(P.degree()) for i in range(Q.degree())]\n");
    }
    else{
      double t, u;
      long j;
