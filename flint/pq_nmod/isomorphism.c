...
void pq_nmod_iso_from_mono(pq_nmod_elt_t res,
  ...
  nmod_poly_trem(y, A->newton->coeffs, B->M, M+n-1);
  ...
  for (n--; n >= 0; n--) {
    ...
    if (!nmod_poly_is_zero(x[n]->dual)) {
      ...
      __COEFF_PROD(tmp, tmp, y+n, A->M->mod, M);
...
void pq_nmod_iso_to_dual(pq_nmod_elt_t* res,
  ...
  for (n--; n >= 0; n--) {
    __COEFF_PROD(res[n]->mono->coeffs, x->mono->coeffs, y+n, A->M->mod, M);
...
void pq_nmod_iso_to_mono(pq_nmod_elt_t* res,
  ...
  for (n--; n >= 0; n--) {
    ...
    y[n] = WORD(0);
    __COEFF_PROD(res[n]->mono->coeffs, x->mono->coeffs, y, A->M->mod, M);
    _nmod_poly_normalise(res[n]->mono);
    nmod_poly_rem(res[n]->mono, res[n]->mono, A->M);
    nmod_poly_zero(res[n]->dual);
  }
