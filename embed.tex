...
\begin{figure}
  ...
  \caption{Timings in seconds for $p=5$, $n=m+1$, $m$ increasing}
  \label{fig:bench}
\end{figure}
...
same fields in Magma.

Our implementation is restricted to finite fields of word-sized
characteristic.  It is based on the C library Flint, and we make it
available as a Sage module in an experimental fork at
...
library, when the code will have stabilized.

Based on the observation that algorithms {\sf Embed} and {\sf Project}
are simpler than those to change between monomial and dual bases, we
chose to implement a \emph{lazy change of basis} strategy. By this we
mean that our Sage module (rather than the C library itself)
represents elements on both the monomial and the dual basis, with one
representation computed from the other only when it is needed the
algorithms. For example, two elements of the same field can be summed
if both have a monomial or else if both have a dual
representation. Similarly, two elements can be multiplied using
standard multiplication if both have a monomial representation, or
using transposed multiplication if one of the two has a monomial
representation. In all other cases, the required representation is
computed and stored at the moment the user asks to compute an
operation. To implement this strategy efficiently, our Sage module is
...
multiplication, this might be due to a poor tuning of Flint. The cost
of {\sf Embed} is about the same as that of multiplication, while {\sf
  DualToMonomial} is about 50\% slower. {\sf Project} and {\sf
  MonomialToDual} have, respectively, similar performances (only
slightly faster) hence they are not reported on the graph. This
justifies our design choice for \emph{lazy change of basis}.
Unsurprisingly, the isomorphism algorithms take significantly more
time than the computation of $R$; {\sf Phi2} is asymptotically faster
than {\sf Phi1} and the switch between the two happens around
$m=70$. In view of this, it might be interesting to use these
algorithms to compute the matrices of $\Phi$ and $\Phi^{-1}$, when
...
fields together with the embeddings, and the time to do the operation
equivalent to {\sf Embed}.
\begin{itemize}
\item In \verb+irred+ we supply directly $P$, $Q$ and $R$ to Magma's
  finite field constructor, then we call the \verb+Embed+ routine to
  compute the embeddings.
\item In \verb+P_R+ we use Magma's default constructor to compute $P$
  and $R$ (Magma chooses its own polynomials), then we call the
  \verb+Embed+ routine to compute the embedding.
\item In \verb+P_Q+ we use Magma's default constructor to compute $P$
  and $Q$ (Magma chooses its own polynomials), then use the
  \verb+CommonOverfield+ routine to compute $R$, then \verb+Embed+ to
  compute the embeddings.
\item In \verb+ext+ we use Magma's default constructor to compute $P$,
