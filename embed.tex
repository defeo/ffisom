...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[1]
    ...
    \ENSURE $a_R^\ast\in k[z]/\ang{R}$ such that $a_R^\ast=\Phi(b_P^\ast c_Q^\ast)$
...
\begin{proof}
  ...
  $\Phi(b_P^\ast c_Q^\ast)$ on the dual basis of $k[z]/\ang{R}$.
...
Let $\rho^\ast\in C$, we can see Embed$(\cdot,\rho^\ast)$ as a mapping
$\epsilon_\rho:B^\ast\to A^\ast$. Hence its dual with respect to the
bilinear forms $\tau_R$ and $\tau_P$ is a map $\epsilon_\rho^t:A\to B$. Let
$a_P^\ast,b_P\in B$, and let $a_R^\ast=\varphi_x(a_P^\ast),
b_R=\varphi_x(b_P)$. Then
\begin{equation}
  \label{eq:transpose-embed}
  \tau_Q(\rho^\ast)\tau_P(a_P^\ast b_P) = \tau_I(\rho^\ast a_R^\ast b_R) = \ang{\rho^\ast a_R^\ast,b_R}_R = \ang{\epsilon_\rho(a_P^\ast),b_R}_R = \ang{a_P^\ast,\epsilon_\rho^t(b_R)}_P.
\end{equation}
...
$\epsilon_\rho^t(b_R)/\tau_Q(\rho^\ast) = b_P$.  In particular, when
$\tau_Q(\rho^\ast)=1$, the map $\epsilon_\rho^t$ is the inverse of
$\phi_x$ on the monomial basis. Observe that it is easy to pick a
$\rho^\ast$ such that $\tau_Q(\rho^\ast)=1$, for example the
\emph{residue form} $(1,0,\dots,0)$ works. We deduce
Algorithm~\ref{algo:inverseEmbed}.

\begin{algorithm}[H]
  \caption{InverseEmbed$(a,\rho)$}
  \begin{algorithmic}[1]
    \REQUIRE $a_R\in k[z]/\ang{R}$, $\rho^\ast\in k[y]/\ang{Q}$ such that $\tau_Q(\rho^\ast)=1$
    \ENSURE $a_P\in k[x]/\ang{P}$ such that $a_R=\varphi_x(a_P)$
    \STATE $(u_i^\ast) = \rem^t(\rho^\ast,Q,mn)$
...
\begin{Lemma}
  Given $a_R$ in the image of $\varphi_x$, and $\rho^\ast$ such that
  $\tau_Q(\rho^\ast)=1$, Algorithm~\ref{algo:inverseEmbed} correctly
  computes $a_P$ such that $a_P=\varphi_x(a_R)$ using $O(\M(mn))$
  operations in $k$.
\end{Lemma}
\begin{proof}
  We show correctness using transposition techniques as
  in~\cite{bostan+lecerf+schost:tellegen}. Observe that for fixed
  $\rho^\ast$ algorithm Embed$(a,\rho^\ast)$ is a linear transform on
  $a$. Precisely, it is equal to $\psi_\rho\circ\rem^t$, where
  $\psi_\rho$ is the map that multiplies a vector in $k^{mn}$
  coefficient-wise by $(\tau_Q(\rho^\ast y^i))_{i<mn}$. Hence, its
  transpose is $\rem\circ\psi_\rho^t$. It is evident that
  $\psi_\rho^t=\psi_\rho$, then $\rem$ is just reduction modulo
  $P$. These correspond to steps~\ref{algo:inverseEmbed:psi}
  and~\ref{algo:inverseEmbed:mod}. Since $\tau_Q(\rho^\ast)=1$, this
  transpose is the inverse of $\phi_x$ on the dual basis.  The cost
  analysis is similar to the one in Lemma~\ref{lemma:algo:embed}.
\end{proof}
