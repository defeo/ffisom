...
most algorithms use linear algebra techniques. In addition, in order
for the system to be consistent, we must ensure that these embeddings
are {\em compatible}~\cite{bosma+cannon+steel97}.
...
$\F_{p^{\ell^j}}$. Provided conversions between these representations
are available, this will allow us to perform embeddings (that will
necessarily be compatible) between different levels of the ``tower'',
\emph{i.e.}\ extensions of degrees $(\ell^i)_{i \ge 1}$.
...
$(2,3]$ such that one can multiply matrices of size $n$ over $A$ using
  $O(n^\omega)$ operations $(+,\times)$ in~$A$; using the algorithms
  of~\cite{coppersmith+winograd,Williams12}, we can take $\omega \le
  2.38$.

Our main result improves on these former ones. Stated with the
notation used so far, it reads as follows. We denote by $\M:\N \to \N$
a function such that for any ring $A$, polynomials in $A[x]$ of degree
at most $n$ can be multiplied in $\M(n)$ operations $(+,\times)$ in
$A$, and we make the usual super-linearity assumptions on
$\M$~\cite[Chapter~8]{vzGG}.
\begin{Theo}
  One can apply $\varphi_x$ (resp.\ $\varphi_x$) to an element of
...
$\bxi=(x^i)_{0 \le i < m}$ and $\bupsilon=(y^i)_{0 \le j < n}$ be the
...
\bupsilon^\ast$ are bases of $A$ as well. The following easy lemma will
...
\otimes \bupsilon^\ast$. After a precomputation of cost $O(\M(m)\log(m) +
...
(or $\varphi_y$) and $\Phi$: modular composition, but none of these
approaches has a quasi-linear running time.
...
\otimes \bupsilon^\ast$ of $A$). We use the further observation that
...
$k[z]/\ang{R}$; the output is written on the basis $\bxi^\ast
\otimes \bupsilon$ of $A$.
...
positive integers, $\K_m$ will be the tensor product
...
the $\ell$-adic tower, we then arrive on the basis $(x_{m'}^i
x_{\ell^{k+1}}^j)_{0 \le i < m', 0 \le j < \ell^{k+1}}$; applying our
isomorphism algorithm, we finish on the basis $(x_{m\ell}^i)_{0 \le i
...
consider the question of computing $\Phi_{(m,n),(mn)}$, that is, going
from the univariate basis $(x_{mn}^i)_{0 \le i < mn}$ to the bivariate
one $(x_m^i x_{mn}^j)_{0 \le i < m, 0 \le j < n}$. This is useful to
compute e.g.  relative minimal polynomials of elements of
$\F_{p^{mn}}$ over $\F_{p^m}$.
